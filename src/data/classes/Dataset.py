...
class Dataset:
    ...
    def df(self) -> pd.DataFrame:
        if self.all_data is None:
            raise Warning('Нет доступа к свойству до выполнения метода process()')
    ...
    def process(self):
        if self.key_var not in self.train_df.columns:
            raise Warning(f'Ключевая переменная {self.key_var} отсутствует в тренировочном датасете')
        if self.log:
    ...
    def __clearEmpty(self):
        ...
        click.echo(click.style('Обработка пропущенных значений в датасете...', fg='green'))
        ...
        for col in nullsValues.head(10).index:
            ...
            null_from_total = str(round((isNull / self.all_data.shape[0]) * 100, 2))
            click.echo(click.style(f'{col} {isNull} ({null_from_total})%\n', fg='green'))
    ...
    def __getDummies(self):
        ...
        self.all_data = pd.get_dummies(self.all_data, 
                                        ...
                                        drop_first=False)
        click.echo(click.style(f'Новый размер датасета {self.all_data.shape}\n', fg='green'))
