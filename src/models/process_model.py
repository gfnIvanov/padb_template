...
@click.option('--rate', 
              type=float,
              help='Learning rate')
def process(csv: str, res: str, tsize: float, estim: int, depth: int, rate: float):
    try:
        ...
        model = xgb.XGBRegressor(n_estimators=estim, max_depth=depth, learning_rate=rate)
        model.fit(X_train, y_train)
        with open(Path.joinpath(root_dir, res, 'XGBmodel.dat'), 'wb') as f:
            pickle.dump(model, f)

        click.echo(click.style(f'Обучение модели завершено. Путь: {res}', fg='green')) 
        click.echo(click.style(f'Качество обучения: {model.score(X_test, y_test)}', fg='green'))
